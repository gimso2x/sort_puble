...
(function($) {
    ...
    $.extend(Monthpicker.prototype, {
        ...
        _attachments: function(input, inst) {
            ...
            if (showOn == 'focus' || showOn == 'both') input.focus(this._showMonthpicker);
            if (showOn == 'button' || showOn == 'both') {
                ...
                inst.trigger.click(function() {
                    if ($.monthpicker._monthpickerShowing && $.monthpicker._lastInput == input[0]) $.monthpicker._hideMonthpicker();
                    else $.monthpicker._showMonthpicker(input[0]);
                    return false
                })
            }
        },
        ...
        _showMonthpicker: function(input) {
            ...
            if ($.monthpicker._isDisabledMonthpicker(input) || $.monthpicker._lastInput == input) return;
            ...
            if ($.monthpicker._inDialog) input.value = '';
            if (!$.monthpicker._pos) {
            ...
            if ($.effects && $.effects[showAnim]) inst.dpDiv.show(showAnim, $.monthpicker._get(inst, 'showOptions'), duration, postProcess);
        ...
        _updateMonthpicker: function(inst) {
            ...
            if (inst == $.monthpicker._curInst && $.monthpicker._monthpickerShowing && inst.input && inst.input.is(':visible') && !inst.input.is(':disabled') && inst.input[0] != document.activeElement) inst.input.focus();
            if (inst.yearshtml) {
        ...
        _hideMonthpicker: function(input) {
            ...
            if (this._monthpickerShowing) {
                ...
                if ($.effects && $.effects[showAnim]) inst.dpDiv.hide(showAnim, $.monthpicker._get(inst, 'showOptions'), duration, postProcess);
        ...
        _notifyChange: function(inst) {
            ...
            if (onChange) onChange.apply((inst.input ? inst.input[0] : null), [inst.selectedYear + 1, inst])
        },
        ...
        _generateHTML: function(inst) {
            ...
            var currentDate = this._daylightSavingAdjust((!inst.currentMonth ? new Date(9999, 9, 9) : new Date(inst.currentYear, inst.currentMonth, 1)));
            var html = '';
            ...
            var prev = '<a class="ui-datepicker-prev ui-corner-all" onclick="MP_jQuery_' + dpuuid + '.monthpicker._adjustDate(\'#' + inst.id + '\', -' + stepYears + ', \'Y\');"' + ' title="' + prevText + '"><span class="ui-icon ui-icon-circle-triangle-w">' + prevText + '</span></a>';
            var next = '<a class="ui-datepicker-next ui-corner-all" onclick="MP_jQuery_' + dpuuid + '.monthpicker._adjustDate(\'#' + inst.id + '\', +' + stepYears + ', \'Y\');"' + ' title="' + nextText + '"><span class="ui-icon ui-icon-circle-triangle-e">' + nextText + '</span></a>';
            html += '<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix ui-corner-all">' + prev + next + this._generateYearHeader(inst, drawYear, monthNames, monthNamesShort) + '</div><table class="ui-datepicker-calendar"><tbody>';
            for (var i = 0; i <= 11; i++) {
                if (i % 3 === 0) {
                    html += '<tr>'
                }
        ...
        _generateYearHeader: function(inst, drawYear, monthNames, monthNamesShort) {
            ...
            if (!inst.yearshtml) {
                ...
                inst.yearshtml += '<select class="ui-datepicker-year" ' + 'onchange="MP_jQuery_' + dpuuid + '.monthpicker._selectYear(\'#' + inst.id + '\', this, \'Y\');" ' + '>';
        ...
        parseDate: function(format, value, settings) {
            ...
            if (date.getFullYear() != year || date.getMonth() + 1 != month || date.getDate() != 1) throw 'Invalid date';
            return date
        },
